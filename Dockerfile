...
ENV REQUIRED_QUORUM=2

RUN case "${TARGETPLATFORM}" in \
      "linux/amd64")   echo "bitcoin-${VERSION}rc1-x86_64-linux-gnu.tar.gz"    > /tarball-name ;; \
      "linux/arm64")   echo "bitcoin-${VERSION}rc1-aarch64-linux-gnu.tar.gz"   > /tarball-name ;; \
      "linux/riscv64") echo "bitcoin-${VERSION}rc1-riscv64-linux-gnu.tar.gz"   > /tarball-name ;; \
      *) echo "Unsupported platform: ${TARGETPLATFORM}" && exit 1 ;; \
    esac

RUN wget https://test.bitcoinknots.org/~luke-jr/programs/bitcoin/files/bitcoin-knots/${PATH_VERSION}/${VERSION}/test/rc1/$(cat /tarball-name) \
         https://test.bitcoinknots.org/~luke-jr/programs/bitcoin/files/bitcoin-knots/${PATH_VERSION}/${VERSION}/test/rc1/SHA256SUMS.asc \
         https://test.bitcoinknots.org/~luke-jr/programs/bitcoin/files/bitcoin-knots/${PATH_VERSION}/${VERSION}/test/rc1/SHA256SUMS
